...
spark = SparkSession.builder.appName("redis-stedi-join-spark").getOrCreate()
...
emailAndBirthDayStreamingDF = spark.sql("select customer, email, phone, birthDay from CustomerRecords where email is not null and birthDay is not null")

# TO-DO: Split the birth year as a separate field from the birthday
emailAndBirthDayStreamingDF = emailAndBirthDayStreamingDF.select('email', 'birthDay', 
...
stediEventsDF\
    .withColumn("value", from_json("value", eventJSONSchema))\
    .select(col('value.*'))\
    .createOrReplaceTempView("CustomerRisk")

# TO-DO: execute a sql statement against a temporary view, selecting the customer and the score from the temporary view, creating a dataframe called customerRiskStreamingDF
customerRiskStreamingDF = spark.sql("select customer, score from CustomerRisk where customer is not null")

# TO-DO: join the streaming dataframes on the email address to get the risk score and the birth year in the same dataframe
redisStediDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("customer = email"))
...
redisStediDF.selectExpr("cast(customer as string) as key", "to_json(struct(*)) as value") \
            .writeStream \
            .format("kafka") \
            .option("kafka.bootstrap.servers", "localhost:9092")\
            .option("topic", "balance-score")\
